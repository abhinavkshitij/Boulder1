...
z = 2*cos(10*x) + cos(20*y)+ 2*cos(18*z)+ 2*cos(15*x+16*y+15*z)+ 0.3; 
%z = 2*cos(10*x) + cos(20*y)+ 2*cos(18*z)+0.3; 
...
% Mesh plot:
subplot(232);
stem(abs(F_fft(1,:,1)),'marker','none');
...
LES = zeros(Fs,Fs,Fs); %Initialize the filter as 1
...
for j=1:Fs
    for k=1:Fs
       if (sqrt((i-center)^2+(j-center)^2+(k-center)^2) <= filter )
            LES(i,j,k) = 1;
        end
    end
end
